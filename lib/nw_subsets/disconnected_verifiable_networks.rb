...
module Netomox
  module Topology
    ...
    class DisconnectedVerifiableNetworks < Networks
      # @return [ModelConductor::NetworkSets] Found network sets
      def find_all_network_sets
        ModelConductor::NetworkSets.new(@networks)
      end
    ...
    class DisconnectedVerifiableNetwork < Network
      ...
      # @return [ModelConductor::NetworkSet] Network-set (set of network-subsets, in a network(layer))
      def find_all_subsets
        remove_deleted_state_elements!
        network_set = ModelConductor::NetworkSet.new(@name)
        ...
        @nodes.each do |node|
          ...
          if node.termination_points.empty?
            network_set.subsets.push(ModelConductor::NetworkSubset.new(node.path))
            next
          end

          # if the node has link(s), search connected element recursively
          network_subset = ModelConductor::NetworkSubset.new
      ...
      # @param [ModelConductor::NetworkSet] network_set
      # @return [ModelConductor::NetworkSet]
      ...
      # @param [ModelConductor::NetworkSubset] subset
