...
module ModelConductor
  # Splice layer3 preallocated (empty) resources into specified network/snapshot topology
  class Layer3PreallocatedResourceSplicer
    # @param [Hash] topology_data Topology data to splice
    # @param [Hash] l3e_topology_data Topology data of layer3 preallocated resources (RFC8345 Hash)
    ...
    # @param [Netomox::PseudoDSL::PNode] layer3e_node preallocated (empty) L3 node (duplicated)
    # @param [Netomox::PseudoDSL::PTermPoint] layer3e_tp preallocated L3 tp (in layer3e_node)
    def add_preallocated_tp_for_dup_node(layer3_target_node, layer3e_node, layer3e_tp)
      layer3_dup_tp = layer3_target_node.find_tp_by_name(layer3e_tp.name)
      unless layer3_dup_tp.nil?
        raise StandardError, "Found duplicate preallocated resource: #{layer3e_node.name}[#{layer3e_tp.name}]"
      end
    ...
    # @param [Netomox::PseudoDSL::PNetwork] layer3e_nw layer3 network of preallocated (empty) resources
    # @return [void]
    def add_preallocated_node(layer3_nw, layer3e_nw)
      layer3e_nw.nodes.each do |layer3e_node|
        ...
        if layer3_dup_node.nil?
          ...
          layer3e_node.termination_points.each do |layer3e_tp|
            add_preallocated_tp_for_dup_node(layer3_dup_node, layer3e_node, layer3e_tp)
          end
        end
      end
    end
    ...
    # @param [Netomox::PseudoDSL::PNetwork] layer3e_nw layer3 network of preallocated (empty) resources
    # @return [void]
    def add_links_between_preallocated_resource(layer3_nw, layer3e_nw)
      layer3e_nw.links.each do |layer3e_link|
        layer3_nw.links.push(layer3e_link)
      end
    end

    # raise [StandardError] found duplicated object in layer3 between target topology and preallocated-resource
    # @return [void]
    def splice_layer3!
      layer3_nw = @topology.find_network('layer3')
      layer3e_nw = @l3e_topology.find_network('layer3')

      add_preallocated_node(layer3_nw, layer3e_nw)
      add_links_between_preallocated_resource(layer3_nw, layer3e_nw)
    end
  end
end
